...
class QrPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final lightPaint = options.colors.light.createPaint(realCodeSize, realCodeSize);
    final darkPaint = options.colors.dark.createPaint(realCodeSize, realCodeSize);
    Path? darkPath;
    Path? lightPath;
    ...
    final framePathZeroOffset = options.shapes.frame.createPath(Offset.zero, frameSize, Neighbors.empty);
    final framePaint = options.colors.frame.createPaint(pixelSize * 3, pixelSize * 3);

    final ballPathZeroOffset = options.shapes.ball.createPath(Offset.zero, pixelSize * 3, Neighbors.empty);
    final ballPaint = options.colors.ball.createPaint(pixelSize * 3, pixelSize * 3);

    canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height), options.colors.background.createPaint(size.width, size.height));
    ...
    void drawFrame(double dx, double dy) {
      if (options.colors.frame is! QrColorUnspecified) {
        ...
        var path = options.shapes.frame.createPath(Offset(dx, dy), frameSize, Neighbors.empty);
    ...
    void drawBall(double dx, double dy) {
      if (options.colors.ball is! QrColorUnspecified) {
        ...
        var path = options.shapes.ball.createPath(Offset(dx, dy), ballSize, Neighbors.empty);
        canvas.drawPath(path, darkPaint);
      }
    }
    ...
    for (int i = 0; i < matrix.width; i++) {
      for (int j = 0; j < matrix.height; j++) {
        ...
        if (options.colors.dark is! QrColorUnspecified) {
          darkPath = options.shapes.darkPixel.createPath(Offset.zero, pixelSize, matrix.neighbors(i, j));
        }
        if (options.colors.light is! QrColorUnspecified) {
          lightPath = options.shapes.lightPixel.createPath(Offset.zero, pixelSize, matrix.neighbors(i, j));
        }
  ...
  @override
  bool operator ==(Object other) => other is QrPainter && other.data == data && other.options == options;
